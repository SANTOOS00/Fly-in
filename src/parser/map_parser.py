...
class BaseParser(ABC):
    ...
    def parse_metadata(self, meta_string: str) -> Dict[str, str]:
            ...
            if not meta_string.startswith("["):
                raise ParsingError(f"MetaData must start with '['")
            if not meta_string.endswith("]"):
                raise ParsingError(f"MetaData missing closing bracket ']'")
            ...
            if "," in content:
                raise ParsingError(
                    f"Invalid character ',' found in MetaData. "
                    f"Properties must be separated by spaces only."
                )
            pairs = content.split()
            for pair in pairs:
                if "=" not in pair:
                    raise ParsingError(f"Invalid MetaData property '{pair}'. Expected 'key=value'")
                    
                key, value = pair.split("=", 1)
                key = key.strip()
                value = value.strip()
                if not key or not value:
                    raise ParsingError(f"MetaData key or value cannot be empty in '{pair}'")
...
class DroneParser(BaseParser):
    def parse(self) -> Drones:
        try:
            ...
            raise ParsingError(f"'nb_drones' value '{self.raw_line}' must be a valid integer.")
...
class NodeParser(BaseParser):
    def split_line(self) -> Tuple[str, int, int, Optional[str]]:
        ...
        if len(data) < 3:
            raise ParsingError(f"Invalid format. Expected 'name Y X [metadata]'")
    ...
    def validate_common_meta(self, meta_dict: Dict[str, Any]) -> Dict[str, Any]:
        if "max_drones" in meta_dict:
            try:
                meta_dict["max_drones"] = int(meta_dict["max_drones"])
            except ValueError:
                raise ParsingError(f"'max_drones' value must be an integer.")
...
class ConnectionParser(BaseParser):
    def parse(self) -> Connection:
        ...
        if not parts:
            raise ParsingError(f"Empty connection details.")
            
        link_part = parts[0]
        if "-" not in link_part:
            raise ParsingError(f"Connection must follow 'node1-node2' format.")
            
        node1, node2 = link_part.split("-", 1)
        if not node1.strip() or not node2.strip():
            raise ParsingError(f"Connection nodes cannot be empty.")
        ...
        if len(parts) == 2:
            ...
            if "max_link_capacity" in meta_dict:
                try:
                    meta_dict["max_link_capacity"] = int(meta_dict["max_link_capacity"])
                except ValueError:
                    raise ParsingError(f"'max_link_capacity' must be an integer.")
            for k in meta_dict:
                if k != "max_link_capacity":
                    raise ParsingError(f"Invalid connection metadata key '{k}'.")
...
class LineValidator:
    ...
    def next_parser(self) -> Tuple[Optional[BaseParser], Optional[str]]:
        ...
        if ":" not in clean_line:
            raise ParsingError(f"Syntax error. Expected 'key: value' structure.")
...
class ConfigParser:
    ...
    def print_report(self) -> bool:
        if self.errors:
            string_error = ""
            print(f"\n💥 Found {len(self.errors)} Error(s) in configuration file:")
    ...
    def validate_structural_rules(self) -> None:
        if self.drones is None:
            self.errors.append("Global Error: Missing 'nb_drones' definition in the file.")
        if self.start_hub is None:
            self.errors.append("Global Error: Missing 'start_hub' definition. The simulation needs a starting point.")
        if self.end_hub is None:
            self.errors.append("Global Error: Missing 'end_hub' definition. The simulation needs a destination point (goal).")
